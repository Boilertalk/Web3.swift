...
extension EthereumValue.ValueType: Hashable {
    ...
    public func hash(into hasher: inout Hasher) {
        switch self {
        case .string(let str):
            hasher.combine(hashValues(str))
        case .int(let int):
            hasher.combine(hashValues(String(int)))
        case .bool(let bool):
            hasher.combine(hashValues(bool ? UInt8(0x01) : UInt8(0x00)))
        case .array(let array):
            hasher.combine(hashValues(String(array.reduce(0, { $0 ^ $1.hashValue }))))
        case .nil:
            hasher.combine(hashValues(UInt8(0x00)))
        }
    }
}
